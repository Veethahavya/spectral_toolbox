...
s.model      = 'spherical';                        % geostatistical model for unknowns
s.variance   = 1;                                  % geostatistical parameter for exponential model
s.lambda     = 50;                                 % geostatistical parameter for exponential model
s.nugget     = 0;                                  % nugget effect
s.micro      = 0.5;                                % microscale smoothing parameter (before nugget)

% definition of the grid for the unknowns s
s.n_pts      = 500;                               % number of unknowns in each direction
s.d_pts      = 0.2;                                % grid spacing in each direction
...
y.npts       = 100;                                % number of observations (not required unless used for problem generation)
...
ind = y.indices;
sz = [s.n_pts s.n_pts]
[row,col] = ind2sub(ind,sz);
% y.gridtype   = 'regular';                          % type of measurement grid (regular or irregular)
% y.d_ratio    = 250;                                % resampling ratio (required for regular grids)
% y.x_first    = 125;                                % location of first measurement: grid node number of s (required for regular grids only)
% y.n_pts      = 20;                                 % size of subsampled field (required for regular grids)
