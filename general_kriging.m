...
% uncertain mean, using the function estimate form and a user-defined
...
% S:           unknowns
% B:           base functinos ![stores things to be calculated]
% Y:           measurements
...
se           = find_embedding(s,options); % ![What is Embedding??]
Qse1         = generate_covariance_embedded_first_row(s,se); %FIND
...
if isequal(options.solver,'standard') || isequal(options.estvar,'standard')
    ...
    if isequal(s.opt,'true')
        for i=1:s.opt_itr
            s.opt_params{i} = variogram_opt(Qyy,y,s); %to find the optimal Variogram Parametres
            s.variance = abs(s.opt_params{i}(5));
            s.lambda = [abs(s.opt_params{i}(3)) abs(s.opt_params{i}(4))];
...
switch b.model
  case 'zero'
    b.HX     = [];                                      % no additional rows and columns [lag]
    b.invQbb = [];                                      % no additional rows and columns
    y.rhs    = y.values;                                % use plain measurement values [data]
